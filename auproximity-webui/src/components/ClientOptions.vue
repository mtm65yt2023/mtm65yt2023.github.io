		...
		<v-card class="pa-5">
			...
			<v-form>
				<v-slider
					...
					:readonly="!amhost"
					:disabled="
						$store.state.options.falloffVision || !$store.state.joinedRoom
					"
...
export default class ClientOptions extends Vue {
	updateOptions() {
		this.$socket.client.emit(ClientSocketEvents.SetOptions, {
	...
	get amhost() {
		return this.$store.state.host === this.$store.state.me?.uuid;
	}
}
</script>
